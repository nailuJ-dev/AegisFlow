...
fn capability_for_another_subject_is_rejected() {
    let request = ToolRequest::new("workflow-2", Operation::FileRead, DataLabel::Trusted, "/safe/input.txt");
    let capability = Capability::issue(Operation::FileRead, "workflow-1", 60).unwrap();
    assert!(!PolicyEngine::default().evaluate(&request, &[capability]).allowed);
}
